(function($){
  Drupal.behaviors.dataset_quarter_filter = {
	...
	// Some non-quarterly-report related functionality for content type date fields -
	// we should either break this out into a separate module or just have a more generic
	// module for all of these little fixes
	$('.node-public_outreach-form .start-date-wrapper input[type=text]').change(function(){
	  if(!($('.end-date-wrapper input[type=text]').hasClass('changed'))){
		$('.end-date-wrapper input[type=text]').val($(this).val());
		$('.end-date-wrapper input[type=text]').removeClass('changed');
	  }
	});

	$('.node-public_outreach-form .end-date-wrapper input[type=text]').change(function(){
